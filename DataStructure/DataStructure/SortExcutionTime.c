...
int main(void) {
	...
	//SelectionSort(a, n);
	t2 = pclock();
	printf("\n%d개 데이터 선택정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	//SelectionSort(a, n);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 선택정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	//InsertionSort(b, n);
	t2 = pclock();
	printf("\n%d개 데이터 삽입정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	//InsertionSort(b, n);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 삽입정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	//BubbleSort(c, n);
	t2 = pclock();
	printf("\n%d개 데이터 버블정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	//BubbleSort(c, n);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 버블정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	ShellSort(d, n);
	t2 = pclock();
	printf("\n%d개 데이터 쉘정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	ShellSort(d, n);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 쉘정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	QuickSort(e, 0, n - 1);
	t2 = pclock();
	printf("\n%d개 데이터 퀵정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	QuickSort(e, 0, n - 1);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 퀵정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	HeapSort(f, n - 1);
	t2 = pclock();
	printf("\n%d개 데이터 힙정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	HeapSort(f, n - 1);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 힙정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	Division(g, 0, n - 1);
	t2 = pclock();
	printf("\n%d개 데이터 병합정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	Division(g, 0, n - 1);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 병합정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	RadixSort(h, n);
	t2 = pclock();
	printf("\n%d개 데이터 기수정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);

	t1 = pclock();
	RadixSort(h, n);
	t2 = pclock();
	printf("\n%d개 정렬된 데이터 기수정렬 시간 : %.8lf초 소요\n", n, (double)(t2 - t1) / llg);
